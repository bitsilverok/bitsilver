  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/bitsilver/bitsilver "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/bitsilverd src/bitsilver-cli src/bitsilver-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/bitsilver-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/bitsilver-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/bitsilver-core-${tag:1}"
        fi
        URL="https://bitsilver.org/$BIN_PATH/bitsilver-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        curl -O $URL
        tar -zxf "bitsilver-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "bitsilver-${tag:1}"
        rm "bitsilver-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
