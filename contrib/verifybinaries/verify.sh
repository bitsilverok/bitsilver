...
###   bitsilvercore.org and bitsilver.org and compares them.
...
WORKINGDIR="/tmp/bitsilver_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitsilvercore.org"
HOST2="https://bitsilver.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitsilver-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitsilver-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitsilver.org failed to provide signature file, but bitsilvercore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitsilver.org and bitsilvercore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitSilver Core binary release signing key installed?"
   fi
