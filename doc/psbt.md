# PSBT Howto for BitSilver Core

Since BitSilver Core 0.17, an RPC interface exists for Partially Signed BitSilver
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitsilver/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitSilver transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitsilvertalk.org/?topic=279249) transactions.
...
#### Multisig with multiple BitSilver Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
BitSilver Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitsilver-cli` is used.
