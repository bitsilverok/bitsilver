BitSilver Core version 0.19.0.1 is now available from:
...
Upgrading directly from a version of BitSilver Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of BitSilver Core are generally supported.

Compatibility
==============

BitSilver Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use BitSilver Core on unsupported systems.
...
10.10. Additionally, BitSilver Core does not yet change appearance when
macOS "dark mode" is activated.

Users running macOS Catalina may need to "right-click" and then choose "Open"
to open the BitSilver Core .dmg. This is due to new signing requirements
imposed by Apple, which the BitSilver Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running BitSilver Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of BitSilver
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile BitSilver Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in BitSilver Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of BitSilver Core. (#14054)

- To eliminate well-known denial-of-service vectors in BitSilver Core,
  ...
  This prevents BitSilver Core from sending the BIP111 NODE_BLOOM service
  ...
  of BitSilver Core and nodes that have manually enabled BIP37 support,
...
- By default, BitSilver Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how BitSilver Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  BitSilver Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, BitSilver Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 BitSilver address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in BitSilver Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@BitSilver AEAD (jonasschnelli)
...
- #15773 Add BitSilverTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding BitSilver Core (luke-jr)
...
- #15683 Comment for seemingly duplicate LIBBITSILVER_SERVER (Bushstar)
