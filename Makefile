...
CONFIG_CLEAN_FILES = libbitsilverconsensus.pc share/setup.nsi \
...
CONFIG_CLEAN_VPATH_FILES = contrib/filter-lcov.py \
	...
	test/util/bitsilver-util-test.py test/util/rpcauth-test.py
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libbitsilverconsensus.pc.in \
	...
	$(top_srcdir)/src/config/bitsilver-config.h.in \
	$(top_srcdir)/test/config.ini.in \
	$(top_srcdir)/test/functional/test_runner.py \
	$(top_srcdir)/test/util/bitsilver-util-test.py \
...
ZMQ_LIBS = -lzmq
abs_builddir = /root/work/bitsilver
abs_srcdir = /root/work/bitsilver
...
pkgconfig_DATA = libbitsilverconsensus.pc
BITSILVERD_BIN = $(top_builddir)/src/$(BITSILVER_DAEMON_NAME)$(EXEEXT)
BITSILVER_QT_BIN = $(top_builddir)/src/qt/$(BITSILVER_GUI_NAME)$(EXEEXT)
BITSILVER_CLI_BIN = $(top_builddir)/src/$(BITSILVER_CLI_NAME)$(EXEEXT)
BITSILVER_TX_BIN = $(top_builddir)/src/$(BITSILVER_TX_NAME)$(EXEEXT)
BITSILVER_WALLET_BIN = $(top_builddir)/src/$(BITSILVER_WALLET_TOOL_NAME)$(EXEEXT)
BITSILVER_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win64-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = BitSilver-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/bitsilver.icns
...
DIST_CONTRIB = $(top_srcdir)/contrib/bitsilver-cli.bash-completion \
	       $(top_srcdir)/contrib/bitsilver-tx.bash-completion \
	       $(top_srcdir)/contrib/bitsilverd.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/bitsilver.ico \
...
COVERAGE_INFO = baseline.info \
  test_bitsilver_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_bitsilver_coverage.info test_bitsilver.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitsilver.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BitSilver-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) $(DIST_CONTRIB) $(DIST_DOCS) \
	...
	test/functional test/fuzz test/util/bitsilver-util-test.py \
	test/util/data/bitsilver-util-test.json \
...
CLEANFILES = $(OSX_DMG) $(BITSILVER_WIN_INSTALLER)
...
src/config/bitsilver-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitsilver-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitsilver-config.h
$(top_srcdir)/src/config/bitsilver-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitsilver-config.h src/config/stamp-h1
libbitsilverconsensus.pc: $(top_builddir)/config.status $(srcdir)/libbitsilverconsensus.pc.in
...
$(BITSILVER_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITSILVERD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITSILVER_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITSILVER_CLI_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITSILVER_TX_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITSILVER_WALLET_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/bitsilver.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/BitSilver-Qt: all-recursive
	$(MKDIR_P) $(@D)
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM)  $(BITSILVER_QT_BIN) $@
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitSilver-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitSilver-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
deploy: $(BITSILVER_WIN_INSTALLER)

$(BITSILVER_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(BITSILVERD_BIN): FORCE
	$(MAKE) -C src $(@F)

$(BITSILVER_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)

$(BITSILVER_TX_BIN): FORCE
	$(MAKE) -C src $(@F)

$(BITSILVER_WALLET_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_bitsilver.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_bitsilver -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_bitsilver_filtered.info: test_bitsilver.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_bitsilver_filtered.info
...
#test_bitsilver_coverage.info: baseline_filtered.info test_bitsilver_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bitsilver_filtered.info -o $@

#total_coverage.info: test_bitsilver_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bitsilver_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitsilver.coverage/.dirstamp:  test_bitsilver_coverage.info
...
#cov: test_bitsilver.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_bitsilver.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
