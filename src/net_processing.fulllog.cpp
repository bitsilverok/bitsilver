...
#if defined(NDEBUG)
# error "BitSilver cannot be compiled without assertions."
#endif
...
void PeerLogicValidation::FinalizeNode(NodeId nodeid, bool& fUpdateConnectionTime) {
    ...
    LogPrintf( "Cleared nodestate for peer=%d\n", nodeid);
}
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    LogPrintf("stored orphan tx %s (mapsz %u outsz %u)\n", hash.ToString(),
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrintf("Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrintf("Erased %d orphan tx due to expiration\n", nErased);
    }
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    {
        LogPrintf( "%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
        state->fShouldBan = true;
    } else
        LogPrintf( "%s: %s peer=%d (%d -> %d)%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
}
...
{
    {
        ...
        if (vOrphanErase.size()) {
            ...
            LogPrintf("Erased %d orphan tx included or conflicted by block\n", nErased);
        }
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        if (state.fPreferHeaderAndIDs && (!fWitnessEnabled || state.fWantsCmpctWitness) &&
            ...
            LogPrintf( "%s sending header-and-ids %s to peer=%d\n", "PeerLogicValidation::NewPoWValidBlock",
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && !pfrom->HasPermission(PF_NOBAN) && (
        ...
        LogPrintf( "Ignore block request below NODE_NETWORK_LIMITED threshold from peer=%d\n", pfrom->GetId());
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetResult() != TxValidationResult::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/bitsilver/bitsilver/issues/8279 for details.
                assert(recentRejects);
...
bool ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!(pfrom->GetLocalServices() & NODE_BLOOM) &&  (strCommand == NetMsgType::FILTERLOAD || strCommand == NetMsgType::FILTERADD))
    {
       /* if (pfrom->nVersion >= NO_BLOOM_VERSION) 
		{
            LOCK(cs_main);
			LogPrintf("Warning-SSM10-100 nVersion >= NO_BLOOM_VERSION=%d",pfrom->nVersion);
...
//		return true;
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        {
            LOCK(cs_main);
			LogPrintf("Warning-SSM1-100 nVersion=%d",pfrom->nVersion);
    ...
    if (pfrom->nVersion == 0) {
        ...
        LOCK(cs_main);
		LogPrintf( "Warning Must have a version message before anything else -1 %d\n", pfrom->nVersion);
        Misbehaving(pfrom->GetId(), 1);
        return false;
    }
    ...
    if (!pfrom->fSuccessfullyConnected) {
        ...
        LOCK(cs_main);
		LogPrintf( "WarningMust have a verack message before anything else -1 %d\n", pfrom->nVersion);
        Misbehaving(pfrom->GetId(), 1);
        return false;
    }
    ...
    if (strCommand == NetMsgType::INV) {
        ...
        for (CInv &inv : vInv)
        {
            ...
            else
            {
                ...
                if (fBlocksOnly) {
                    LogPrintf( "transaction (%s) inv sent in violation of protocol, disconnecting peer=%d\n", inv.hash.ToString(), pfrom->GetId());
                    pfrom->fDisconnect = true;
                    return true;
    ...
    if (strCommand == NetMsgType::GETBLOCKTXN) {
        ...
        if (pindex->nHeight < ::ChainActive().Height() - MAX_BLOCKTXN_DEPTH) {
            ...
            LogPrintf( "Peer %d sent us a getblocktxn for a block > %i deep\n", pfrom->GetId(), MAX_BLOCKTXN_DEPTH);
            CInv inv;
    ...
    if (strCommand == NetMsgType::TX) {
        ...
        if (!AlreadyHave(inv) &&
            ...
            LogPrintf("AcceptToMemoryPool: peer=%d: accepted %s (poolsz %u txn, %u kB)\n",
                pfrom->GetId(),
                tx.GetHash().ToString(),
    ...
    if (strCommand == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (fImporting || fReindex) {
            LogPrintf( "Unexpected cmpctblock message received from peer %d\n", pfrom->GetId());
            return true;
        }
            ...
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrintf( "Peer sent us compact block we were already syncing!\n");
                        return true;
                    }
                }
    ...
    if (strCommand == NetMsgType::BLOCKTXN)
    {
        ...
        if (fImporting || fReindex) {
            LogPrintf( "Unexpected blocktxn message received from peer %d\n", pfrom->GetId());
            return true;
        }
        ...
        bool fBlockRead = false;
        {
            ...
            if (it == mapBlocksInFlight.end() || !it->second.second->partialBlock ||
                ...
                LogPrintf( "Peer %d sent us block transactions for block we weren't expecting\n", pfrom->GetId());
                return true;
            }
    ...
    if (strCommand == NetMsgType::HEADERS)
    {
        ...
        if (fImporting || fReindex) {
            LogPrintf( "Unexpected headers message received from peer %d\n", pfrom->GetId());
            return true;
        }
    ...
    if (strCommand == NetMsgType::BLOCK)
    {
        ...
        if (fImporting || fReindex) {
            LogPrintf( "Unexpected block message received from peer %d\n", pfrom->GetId());
            return true;
        }
    ...
    if (strCommand == NetMsgType::GETADDR) {
        ...
        if (!pfrom->IsAddrRelayPeer()) {
            LogPrintf( "Ignoring \"getaddr\" from block-relay-only connection. peer=%d\n", pfrom->GetId());
            return true;
        }
    ...
    if (strCommand == NetMsgType::FEEFILTER) {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrintf( "received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), pfrom->GetId());
        }
        return true;
    }
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    {
        LogPrintf( "%s(%s, %u bytes): CHECKSUM ERROR peer=%d\n", __func__,
	...
	{
        LogPrintf("PRO-%s(%s, %u bytes): Exception '%s' (%s) caught\n", __func__, SanitizeString(strCommand), nMessageSize, e.what(), typeid(e).name());
    }
	catch (...)
	{
        LogPrintf("PRO-%s(%s, %u bytes): Unknown exception caught\n", __func__, SanitizeString(strCommand), nMessageSize);
    }

    if (!fRet)
	{
        LogPrintf("PRO-%s(%s, %u bytes) FAILED peer=%d\n", __func__, SanitizeString(strCommand), nMessageSize, pfrom->GetId());
    }
    ...
    if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= ::ChainActive().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogPrintf( "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", pto->GetId(), state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", state.m_chain_sync.m_work_header->GetBlockHash().ToString());
        ...
        if (worst_peer != -1) {
            bool disconnected = connman->ForNode(worst_peer, [&](CNode *pnode) {
                ...
                if (time_in_seconds - pnode->nTimeConnected > MINIMUM_CONNECT_TIME && state.nBlocksInFlight == 0) {
                    ...
                    LogPrintf( "keeping outbound peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n", pnode->GetId(), pnode->nTimeConnected, state.nBlocksInFlight);
                    return false;
                }
            });
            if (disconnected) {
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrintf( "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogPrintf( "%s: sending header %s to peer=%d\n", __func__,
                                vHeaders.front().GetHash().ToString(), pto->GetId());
                    }
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    // Just log for now.
                    if (::ChainActive()[pindex->nHeight] != pindex) {
                        LogPrintf( "Announcing block %s not on main chain (tip=%s)\n",
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrintf( "%s: sending inv peer=%d hash=%s\n", __func__,
        ...
        if (state.m_tx_download.m_check_expiry_timer <= current_time) {
            for (auto it=state.m_tx_download.m_tx_in_flight.begin(); it != state.m_tx_download.m_tx_in_flight.end();) {
                if (it->second <= current_time - TX_EXPIRY_INTERVAL) {
                    LogPrintf( "timeout of inflight tx %s from peer=%d\n", it->first.ToString(), pto->GetId());
