...
const char * const BITSILVER_CONF_FILENAME = "bitsilver.conf";
 ...
 * https://github.com/bitsilver/bitsilver/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitsilver-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "bitsilver";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitSilver
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitSilver
    // Mac: ~/Library/Application Support/BitSilver
    // Unix: ~/.bitsilver
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitSilver";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitSilver";
#else
    // Unix
    return pathRet / ".bitsilver";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITSILVER_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitSilver Core copyright is not removed by accident
    if (copyright_devs.find("BitSilver Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitSilver Core developers";
    }
